...
import { OrderFlowError, upsertAdminCategory } from "@/lib/order-repository";
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const parsed = adminCategorySchema.safeParse({ ...body, id });
    ...
    return NextResponse.json(
      { error: "Nao foi possivel atualizar a categoria." },
      { status: 500 },
    );
  }
}
